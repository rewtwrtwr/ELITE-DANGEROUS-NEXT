...
### FR-7: Clean Release (Build Isolation)

| ID | Требование | Приоритет | Статус |
|----|------------|-----------|--------|
| FR-7.1 | Папки метаданных разработки (`.specify/`, `conductor/`, `specs/`) должны быть исключены из релизных сборок | P0 | ⏳ Pending |
| FR-7.2 | Скрипт сборки (`npm run build`) не должен включать служебные файлы в `dist/` | P0 | ⏳ Pending |
| FR-7.3 | `.npmignore` должен явно исключать папки разработки из npm-пакета | P1 | ⏳ Pending |
| FR-7.4 | `.gitignore` должен содержать правила для папок сборки (`dist/`, `build/`, `release/`) | P0 | ✅ |
| FR-7.5 | Валидация чистоты билда перед публикацией (скрипт `prepublishOnly`) | P1 | ⏳ Pending |
| FR-7.6 | Документирование списка исключаемых файлов в CONTRIBUTING.md | P2 | ⏳ Pending |

**Обоснование:** Согласно принципам Spec-Driven Development, метаданные разработки (спецификации, планы, треки) должны быть изолированы от релизных артефактов. Это обеспечивает:
- Чистоту дистрибутивов
- Безопасность (исключение внутренней документации из публичных сборок)
- Предсказуемость сборок (только необходимые файлы)
