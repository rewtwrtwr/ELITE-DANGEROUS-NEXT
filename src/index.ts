...
import { getConfig, logConfig } from "./utils/config.js";
 ...
 * @deprecated Use getConfig() from utils/config.ts instead
 */
...
async function shutdown(signal: string): Promise<void> {
  ...
  // Step 2: Close database (better-sqlite3 writes directly to disk)
...
async function main(): Promise<void> {
  ...
  logger.info("App", "========================================");
  logger.info("App", "Elite Dangerous NEXT v1.0.0-beta");
  logger.info("App", "========================================");
  ...
  app.get("/health", (_req, res) => {
    res.json({ status: "healthy", version: "1.0.0-beta" });
  });
